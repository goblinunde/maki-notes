...
\renewcommand{\LectureNotesTitle}{Maki梦幻数学讲义LaTeX模板}
\renewcommand{\AuthorInfo}{B站@Maki的完美算术教室}
\renewcommand{\CoverTitle}{Maki梦幻数学讲义LaTeX模板}
...
\begin{document}
	...
	模板以 \texttt{ctexbook} 文档类为基础，采用 XeLaTeX 编译，并通过 \texttt{maki-notes.cls} 的类选项统一管理字体与主题配色。默认情况下，正文会使用更接近 Palatino 数学风格的拉丁正文字体栈，中文部分则优先采用跨平台稳定的预设；若需要，也可切换为面向 Windows、macOS 或 Linux 的字体预设。章节标题以楷书排印。数学环境方面，定义、定理、例题、真题等均以 \texttt{tcolorbox} 实现彩色盒子排版，配色各异、层次分明；另设填空、选择等题型辅助命令，可直接用于试卷与讲义编排。讲义图片均以 TikZ 及 \texttt{pgfplots} 绘制，力求清晰可控。
